...
[![PyTorch 2.6+](https://img.shields.io/badge/torch-2.6+-green?logo=pytorch&logoColor=white)](https://pytorch.org/)
[![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/BF667-IDLE/AudioEditingCode/blob/main/colab_notebook.ipynb)
...
This fork has been updated for **Python 3.10+** and **PyTorch 2.6+** compatibility, and includes a **Google Colab notebook** for easy GPU-based usage.
...
**2025-04-08** *(this fork)*: Updated for Python 3.10+ and PyTorch 2.6+ compatibility.

- Fixed `torch.load()` calls with `weights_only=False` for PyTorch 2.6+
- Updated `torch.cuda.amp.autocast` → `torch.amp.autocast("cuda")` with fallback
- Wrapped `allow_fp16_reduced_precision_reduction` in try/except (removed in PyTorch 2.5+)
- Guarded `set_float32_matmul_precision` with `hasattr` check
- Relaxed `librosa` version, unpinned torch dependencies
- Added CUDA availability checks with CPU fallback in all main scripts
- Fixed `torch.FloatTensor` type hints to `torch.Tensor`
- Added `colab_notebook.ipynb` for Google Colab usage
- Relaxed `numpy` and `huggingface-hub` version constraints
...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/BF667-IDLE/AudioEditingCode/blob/main/colab_notebook.ipynb)

You can run all audio editing workflows directly in Google Colab with a free or pro GPU:

- **Text-Based Editing** (DDPM inversion) — edit audio using text prompts
- **Unsupervised Editing** (PC extraction & drift) — discover and apply semantic directions
- **SDEdit** — stochastic editing with configurable noise strength

Click the badge above to open the notebook, or launch it from the command line:
...
The notebook handles dependency installation, model downloading, audio upload, playback, and result download automatically.
