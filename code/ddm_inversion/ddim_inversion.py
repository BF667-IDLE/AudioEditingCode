...
def next_step(ldm_model, model_output: Union[torch.Tensor, np.ndarray],
              timestep: int, sample: Union[torch.Tensor, np.ndarray]):
...
def text2image_ldm_stable(ldm_model, prompt: List[str], num_inference_steps: int = 50,
                          guidance_scale: float = 7.5, xt: Optional[torch.Tensor] = None, skip: int = 0):
